...
class Finder extends Object
{
    ...
    public function findModel($key, $value, $multiple = false)
    {
        if ($key == 'username') {
            return $this->findUserWithProfileByUsername($value);
        }
    ...
    /**
     * Finds a user by the given last active timestamp.
     *
     * @param int $activity_at timestamp
     * @return models\User
     */
    public function findUsersByActivity($activity_at)
    {
        return $this->findUser(['>=', 'activity_at', $activity_at])
            ->orderBy(['activity_at' => SORT_DESC])->all();
    }
    ...
    /**
     * Finds a user with profile by username.
     *
     * @param int $id
     * @return null|models\Profile
     */
    public function findUserWithProfileByUsername($username)
    {
        $dependency = new DbDependency([
            'sql' => User::CACHE_DEPENDENCY_UPDATED_AT,
            'reusable' => true
        ]);
        return User::getDb()->cache(function ($db) use ($username) {
            return $this->userQuery->with('profile')->where(['username' => $username])->one();
        }, null, $dependency);
    }
